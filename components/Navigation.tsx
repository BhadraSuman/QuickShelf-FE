...
export default function Navigation() {
    ...
    return (
        <nav className="sticky top-0 z-50 w-full bg-white shadow-lg border-b-2 border-olive/10">
            <div className="px-4 sm:px-6 lg:px-8 py-4 md:py-5">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    {/* Logo */}
                    <a href="/" className="flex items-center gap-3">
                        <div className="w-11 h-11 bg-gradient-to-br from-olive to-yellow-500 rounded-xl flex items-center justify-center shadow-lg">
                            <span className="text-white font-black text-2xl">⚡</span>
                        </div>
                        <span className="text-2xl sm:text-3xl font-black text-olive tracking-tight">Quickshelf</span>
                    </a>
                    ...
                    <ul className="hidden sm:flex gap-10 text-base sm:text-lg font-bold">
                        ...
                        <li>
                            <a
                                href="/contact"
                                className="px-6 py-2 rounded-lg bg-yellow-400 text-olive font-bold shadow-md hover:shadow-lg hover:bg-yellow-500 transition-all"
                            >
                                Contact
                            </a>
                        </li>
            ...
            {isOpen && (
                <>
                    {/* Backdrop */}
                    <div className="fixed inset-0 bg-black/40 z-40 sm:hidden" onClick={() => setIsOpen(false)}></div>

                    {/* Sidebar */}
                    <div className="fixed top-0 right-0 h-screen w-64 bg-white shadow-2xl z-50 sm:hidden animate-slide-in">
                        {/* Close Button */}
